...
import { users, quacks, books, authors } from './mocks';

const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
const MOCK_DATA_DELAY = 300;
const QUACKS_LIMIT = 20;
const AUTHORS_LIMIT = 20;
...
export default {
  Query: {
    ...
    async books() {
      return books.slice(0, BOOKS_LIMIT);
    },
    async book(_,{id}) {
      return books.find((book) => book.id === id)
    },
    async authors() {
      return authors.slice(0, AUTHORS_LIMIT);
    },
    async author(_, {id}) {
  ...
  Book: {
    authors({ id }) {
      ...
      return authors.filter((author) => book.authorsId.includes(author.id));
    },
  },
  Author: {
    books({ id }) {
      return books
        .filter((book) => book.authorsId.includes(id));
    },
  },
};
